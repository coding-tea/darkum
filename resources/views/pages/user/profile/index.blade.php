    ...
    <form class="p-3" method="post" action="{{ route('profile.update',Auth::user()->id) }}">
      ...
      @method('PUT')
        ...
        <x-form-input title='email' name='email' type='email' value='{{ $user->email }}' />
            <x-form-input title='full name' name='fullName' value='{{ (!empty($data)) ? $data->fullName : ""}}' />
            <x-form-input title='adresse' name='adresse' value='{{ (!empty($data)) ? $data->adresse : ""}}' />
            <x-form-input title='phone number' name='tel' value='{{ (!empty($data)) ? $data->tel : ""}}' />
