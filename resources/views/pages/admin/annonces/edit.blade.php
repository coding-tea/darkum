...
<div class="card shadow mb-4">
    ...
    <form class="p-3" method="post" action="{{ route('annonces.update', $announce->id) }}">
      @csrf
      @method('PUT')
          ...
          <div class="mb-3">
            ...
            <select id="typel" name="typeL" width='100%' required>
              @foreach ($typeL as $item)
                  <option value="{{ $item }}" {{ ($announce == $item)?'selected' : '' }} > {{ $item }} </option>
              @endforeach
            </select>
          </div>
          @endisset
          ...
          <div class="mb-3">
            ...
            <select id="type" name="type" width='100%' required>
              @foreach ($type as $item)
                  <option value="{{ $item }}" {{ ($announce->type == $item)?'selected' : '' }} > {{ $item }} </option>
              @endforeach
            </select>
          </div>
          @endisset
